...
def aug_transform(crop, base_transform, cfg, extra_t=[]):
    ...
    if cfg.blur_kernel_size < 3 or cfg.blur_kernel_size % 2 != 1:
        raise ValueError("blur kernel size must be odd and at least three")
    ...
    transforms = [
            ...
            T.RandomApply(
                [T.GaussianBlur(kernel_size=cfg.blur_kernel_size, sigma=(0.1, 2.0))],
                p=cfg.blur_prob,
            ),
        ]
