...
def apply_backbone_initialization(
    ...
    *,
    ...
    base_conv1_mode: str,
    mode: str,
    stl_maxpool: str = "enabled",
) -> dict:
    if dataset not in {"cifar10", "cifar100", "stl10", "tinyimagenet"}:
        raise ValueError(
            "this initialization suite is restricted to CIFAR/STL/Tiny ImageNet"
        )
    ...
    if stl_maxpool not in {"enabled", "disabled"}:
        raise ValueError(f"unknown STL-10 maxpool mode: {stl_maxpool}")
    ...
    if conv1.bias is not None:
        raise RuntimeError("replacement conv1 unexpectedly has a bias")
    if dataset in {"cifar10", "cifar100"}:
        if not isinstance(backbone.maxpool, nn.Identity):
            raise RuntimeError("frozen CIFAR maxpool structure changed")
    elif stl_maxpool == "disabled":
        if not isinstance(backbone.maxpool, nn.Identity):
            raise RuntimeError("disabled STL-10 maxpool is not Identity")
    else:
        pool = backbone.maxpool
        if not (
            isinstance(pool, nn.MaxPool2d)
            and pool.kernel_size == 3
            and pool.stride == 2
            and pool.padding == 1
            and pool.dilation == 1
            and pool.ceil_mode is False
        ):
            raise RuntimeError("frozen STL-10/Tiny ImageNet maxpool structure changed")
...
def make_get_model(
    ...
    *,
    ...
    def wrapped(arch, dataset, init_mode="pretrained", *args, **kwargs):
        if init_mode != "random":
            raise ValueError("from-scratch initialization requires --init-mode random")
        requested_stl_maxpool = kwargs.get(
            "stl_maxpool", args[0] if args else "enabled"
        )
        model, out_size = original(arch, dataset, init_mode, *args, **kwargs)
        ...
        event = apply_backbone_initialization(
            ...
            base_conv1_mode=base_conv1_mode,
            mode=mode,
            stl_maxpool=requested_stl_maxpool,
        )
        if emit_event:
